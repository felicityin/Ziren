...
pub enum ShardVerifierError<EF, PcsError> {
    ...
    #[error("preprocessed chip id mismatch: {0}, {1}")]
    ...
    GC::F: PrimeField32,
{
    ...
    pub fn verify_zerocheck(
        ...
        // Verify the zerocheck proof.
        partially_verify_sumcheck_proof(&proof.zerocheck_proof, challenger, max_log_row_count, MAX_CONSTRAINT_DEGREE + 1)
            .map_err(ShardVerifierError::<_, <SC::Config as MultilinearPcsVerifier<GC>>::VerifierError>::ConstraintsCheckFailed)?;
    ...
    pub fn verify_shard(
        ...
        // Remove the last two row counts because we add the padding columns as two extra tables.
        for _ in 0..2 {
...
impl<A> ShardVerifier<ZkmGlobalContext, ZkmSC<A>>
where
    A: ZerocheckAir<<ZkmGlobalContext as IopCtx>::F, <ZkmGlobalContext as IopCtx>::EF>,
    <ZkmGlobalContext as IopCtx>::F: TwoAdicField,
    <ZkmGlobalContext as IopCtx>::EF: TwoAdicField,
{
