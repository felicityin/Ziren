use std::{collections::{BTreeMap, BTreeSet}, fmt::Debug, future::Future, iter::once, sync::Arc};

use derive_where::derive_where;
use itertools::Itertools;
use serde::{Deserialize, Serialize};
use slop_air::Air;
use slop_algebra::{Field, FieldAlgebra};
use slop_alloc::{CanCopyFromRef, CpuBackend};
...
use slop_multilinear::{Evaluations, MleEval, MultilinearPcsProver, MultilinearPcsVerifier, Point, VirtualGeq};
...
use crate::{
    ...
    prover::{DefaultTraceGenerator, ProverPermit, ProverSemaphore, Record, TraceData},
    septic_digest::SepticDigest,
    AirOpenedValues, Chip, ChipEvaluation, ChipOpenedValues, ChipStatistics, ConstraintSumcheckFolder, GkrProverImpl,
    LogUpEvaluations, Machine, MachineVerifyingKey, ShardContext, ShardOpenedValues, ShardProof, ZeroCheckPoly, ZerocheckCpuProverData,
};

use super::{PreprocessedTraceData, TraceGenerator, Traces};
...
pub type Program<GC, SC> = <<SC as ShardContext<GC>>::Air as MachineAir<<GC as IopCtx>::F>>::Program;
...
pub struct ShardData<GC: IopCtx, SC: ShardContext<GC>, C: DefaultJaggedProver<GC, SC::Config>> {
    ...
    /// Main trace data
    pub main_trace_data: crate::prover::MainTraceData<GC::F, SC::Air, CpuBackend>,
}
...
pub struct ShardProverInner<GC: IopCtx, SC: ShardContext<GC>, C: MultilinearPcsProver<GC, PcsProof<GC, SC>>> {
    /// The trace generator.
...
pub struct ShardProver<GC: IopCtx, SC: ShardContext<GC>, C: MultilinearPcsProver<GC, PcsProof<GC, SC>>> {
...
impl<GC: IopCtx, SC: ShardContext<GC>, C: MultilinearPcsProver<GC, PcsProof<GC, SC>>> Clone for ShardProver<GC, SC, C> {
    fn clone(&self) -> Self {
        Self { inner: Arc::clone(&self.inner) }
    }
}

impl<GC: IopCtx, SC: ShardContext<GC>, C: MultilinearPcsProver<GC, PcsProof<GC, SC>>> ShardProver<GC, SC, C> {
...
impl<GC: IopCtx, SC: ShardContext<GC>, C: DefaultJaggedProver<GC, SC::Config>> AirProver<GC, SC> for ShardProver<GC, SC, C> {
    ...
    async fn setup_from_vk(
        ...
        if let Some(vk) = vk {
            ...
            self.setup_with_initial_global_cumulative_sum(program, initial_global_cumulative_sum, prover_permits).await
        } else {
            let program_sent = program.clone();
            let initial_global_cumulative_sum =
                tokio::task::spawn_blocking(move || program_sent.initial_global_cumulative_sum()).await.unwrap();
            self.setup_with_initial_global_cumulative_sum(program, initial_global_cumulative_sum, prover_permits).await
        }
    }
    ...
    async fn setup_and_prove_shard(
        ...
        let (pk, vk) = {
            ...
            self.setup_from_preprocessed_data_and_traces(pc_start, initial_global_cumulative_sum, preprocessed_traces)
        };
    ...
    async fn preprocessed_table_heights(pk: Arc<ProvingKey<GC, SC, Self>>) -> BTreeMap<String, usize> {
        std::future::ready(
            pk.preprocessed_data.preprocessed_traces.iter().map(|(name, trace)| (name.to_owned(), trace.num_real_entries())).collect(),
        )
        .await
    }
}

impl<GC: IopCtx, SC: ShardContext<GC>, C: DefaultJaggedProver<GC, SC::Config>> ShardProver<GC, SC, C> {
    /// Get all the chips in the machine.
    #[must_use]
    ...
    pub fn setup_from_preprocessed_data_and_traces(
        ...
        let (preprocessed_commit, preprocessed_data) = self.inner.pcs_prover.commit_multilinears(message).unwrap();

        let vk = MachineVerifyingKey { pc_start, initial_global_cumulative_sum, preprocessed_commit };
    ...
    pub async fn setup_with_initial_global_cumulative_sum(
        ...
        let preprocessed_data =
            self.inner.trace_generator.generate_preprocessed_traces(program, self.max_log_row_count(), setup_permits).await;

        let PreprocessedTraceData { preprocessed_traces, permit } = preprocessed_data;

        let (pk, vk) = self.setup_from_preprocessed_data_and_traces(pc_start, initial_global_cumulative_sum, preprocessed_traces);
    ...
    fn commit_traces(&self, traces: &Traces<GC::F, CpuBackend>) -> (GC::Digest, JaggedProverData<GC, C::ProverData>) {
    ...
    fn zerocheck(
        ...
        let max_num_constraints = itertools::max(chips.iter().map(|chip| chip.num_constraints)).unwrap();
        let powers_of_challenge = batching_challenge.powers().take(max_num_constraints).collect::<Vec<_>>();
        let airs = chips.iter().map(|chip| (chip.air.clone(), chip.num_constraints)).collect::<Vec<_>>();
        ...
        for ((air, num_constraints), chip) in airs.iter().cloned().zip_eq(chips.iter()) {
            let ChipEvaluation { main_trace_evaluations: main_opening, preprocessed_trace_evaluations: prep_opening } =
            ...
            let threshold_point = Point::from_usize(num_real_entries, self.inner.pcs_prover.max_log_row_count + 1);
            ...
            let gkr_opening_batch_randomness_powers = gkr_opening_batch_randomness
                .powers()
                .skip(1)
                .take(main_opening.num_polynomials() + prep_opening.as_ref().map_or(0, MleEval::num_polynomials))
                .collect::<Vec<_>>();
            let gkr_powers = Arc::new(gkr_opening_batch_randomness_powers);

            let alpha_powers = Arc::new(chip_powers_of_alpha);
            let air_data = ZerocheckCpuProverData::round_prover(air, public_values.clone(), alpha_powers, gkr_powers.clone());
            ...
            let chip_sumcheck_claim = main_opening
                ...
                .as_slice()
                .iter()
                .chain(prep_opening.as_ref().map_or_else(Vec::new, |mle| mle.evaluations().as_slice().to_vec()).iter())
                .zip(gkr_powers.iter())
                .map(|(opening, power)| *opening * *power)
                .sum::<GC::EF>();

            let initial_geq_value = if main_trace.num_real_entries() > 0 { GC::EF::zero() } else { GC::EF::one() };
        ...
        let (partial_sumcheck_proof, component_poly_evals) =
            reduce_sumcheck_to_evaluation(zerocheck_polys, challenger, chip_sumcheck_claims, 1, lambda);
        ...
        let shard_open_values = airs
            ...
            .map(|((air, _), evals)| {
                ...
                (air.name().to_string(), ChipOpenedValues { preprocessed, main, degree: chip_heights[&air.name()].clone() })
            })
    ...
    #[allow(clippy::type_complexity)]
    pub fn prove_shard_with_data(&self, data: ShardData<GC, SC, C>, mut challenger: GC::Challenger) -> (ShardProof<GC, PcsProof<GC, SC>>, ProverPermit) {
        let ShardData { pk, main_trace_data } = data;
        let crate::prover::MainTraceData { traces, public_values, shard_chips, permit } = main_trace_data;
        ...
        let round_evaluation_claims =
            preprocessed_evaluation_claims.into_iter().chain(once(main_evaluation_claims)).collect::<Rounds<_>>();

        let round_prover_data = once(pk.preprocessed_data.preprocessed_data.clone()).chain(once(main_data)).collect::<Rounds<_>>();
...
pub struct ShardProverData<GC: IopCtx, SC: ShardContext<GC>, C: MultilinearPcsProver<GC, PcsProof<GC, SC>>> {
