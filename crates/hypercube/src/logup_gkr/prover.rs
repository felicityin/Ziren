...
/// A prover for the `LogUp` GKR circuit.
...
impl<GC: IopCtx, SC: ShardContext<GC>> GkrProverImpl<GC, SC> {
    ...
    pub fn prove_gkr_circuit(
        ...
        while let Some(layer) = circuit.next_layer() {
            let round_proof = prove_gkr_round(layer, &eval_point, numerator_eval, denominator_eval, challenger);
    ...
    pub(crate) fn prove_logup_gkr(
        ...
        let max_public_values_interaction_arity = Record::<GC, SC>::interactions_in_public_values()
            .iter()
        ...
        let num_interactions = chips.iter().map(|chip| chip.sends().len() + chip.receives().len()).sum::<usize>();
        ...
        let (output, circuit) = {
            ...
            self.trace_generator.generate_gkr_circuit(chips, preprocessed_traces.clone(), traces.clone(), public_values, alpha, beta_seed)
        };
        ...
        let (eval_point, round_proofs) = {
            ...
            self.prove_gkr_circuit(first_numerator_eval, first_denominator_eval, first_eval_point, circuit, challenger)
        };

        // Get the evaluations for each chip at the evaluation point of the last round.
        let mut chip_evaluations = std::collections::BTreeMap::new();
        ...
        for chip in chips.iter() {
            ...
            let preprocessed_evaluation = preprocessed_trace.as_ref().map(|t| t.eval_at_eq(&eval_point, &eval_point_eq));
            ...
            let openings =
                ChipEvaluation { main_trace_evaluations: main_evaluation, preprocessed_trace_evaluations: preprocessed_evaluation };
