...
use crate::{lookup::Lookup, prover::Traces};
...
impl<F: Field, EF: ExtensionField<F>, A> LogupGkrCpuTraceGenerator<F, EF, A> {
    ...
    pub(crate) fn extract_outputs(&self, last_layer: &LogUpGkrCpuLayer<EF, EF>) -> LogUpGkrOutput<EF> {
        ...
        let mut numerator_0_interactions: Vec<EF> = numerator_0
            ...
            .collect();
        numerator_0_interactions.resize(1 << (last_layer.num_interaction_variables + 1), EF::zero());
        let mut numerator_1_interactions: Vec<EF> = numerator_1
            ...
            .collect();
        numerator_1_interactions.resize(1 << (last_layer.num_interaction_variables + 1), EF::zero());
        let mut denominator_0_interactions: Vec<EF> = denominator_0
            ...
            .collect();
        denominator_0_interactions.resize(1 << (last_layer.num_interaction_variables + 1), EF::one());
        let mut denominator_1_interactions: Vec<EF> = denominator_1
            ...
            .collect();
        denominator_1_interactions.resize(1 << (last_layer.num_interaction_variables + 1), EF::one());
    ...
    pub(crate) fn generate_first_layer(
        &self,
        interactions: &BTreeMap<String, Vec<(&Lookup<F>, bool)>>,
                ...
                match preprocessed_trace {
                        ...
                        numer_evals
                            ...
                            .zip_eq(
                                prep.inner()
                                    ...
                                    .zip(main_trace.inner().as_ref().unwrap().guts().as_slice().par_chunks(main_trace.num_polynomials())),
                            )
                    ...
                    None => {
                        numer_evals
                            .par_chunks_exact_mut(num_interactions)
                            .zip_eq(denom_evals.par_chunks_exact_mut(num_interactions))
                            .zip_eq(main_trace.inner().as_ref().unwrap().guts().as_slice().par_chunks(main_trace.num_polynomials()))
    ...
    #[allow(clippy::too_many_lines)]
    #[allow(clippy::unused_self)]
    pub(crate) fn layer_transition<K>(&self, layer: &LogUpGkrCpuLayer<K, EF>) -> LogUpGkrCpuLayer<EF, EF>
    where
        ...
        for (n0_padded, d0_padded, n1_padded, d1_padded) in itertools::izip!(
            ...
            if let Some(n0_mle) = n0_padded.inner().as_ref() {
                ...
                n0_mle
                    ...
                    .for_each(
                        |(
                            ((((((n0_chunk, d0_chunk), n1_chunk), d1_chunk), next_n0_row), next_d0_row), next_n1_row),
                            next_d1_row,
                        )| {
                            ...
                            if n0_chunk.len() == 2 * num_interactions {
                                n_10_row
                                    ...
                                    .for_each(|(((((n_10, d_10), n_11), d_11), next_n1), next_d1)| {
