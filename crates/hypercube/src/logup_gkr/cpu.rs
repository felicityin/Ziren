...
impl<F: Field, EF: ExtensionField<F>, A: MachineAir<F>> LogupGkrCpuTraceGenerator<F, EF, A> {
    ...
    pub(crate) fn generate_gkr_circuit(
        ...
        let first_layer =
            self.generate_first_layer(&interactions, &traces, &preprocessed_traces, alpha, beta_seed);
...
pub(crate) fn prove_gkr_round<F: Field, EF: ExtensionField<F>, Challenger: FieldChallenger<F>>(
    ...
    let (numerator_0, denominator_0, numerator_1, denominator_1, sumcheck_proof) = match circuit {
        GkrCircuitLayer::Layer(layer) => {
            let (interaction_point, row_point) = eval_point.split_at(layer.num_interaction_variables);
            ...
            let [numerator_0, denominator_0, numerator_1, denominator_1] = openings.try_into().unwrap();
    ...
    super::LogupGkrRoundProof { numerator_0, numerator_1, denominator_0, denominator_1, sumcheck_proof }
}
