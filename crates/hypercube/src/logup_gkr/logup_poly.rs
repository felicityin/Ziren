...
use rayon::prelude::*;
use slop_algebra::{interpolate_univariate_polynomial, ExtensionField, Field, UnivariatePolynomial};
...
impl<K: ExtensionField<F>, F: Field> SumcheckPolyFirstRound<K> for LogupRoundPolynomial<F, K> {
    ...
    fn fix_t_variables(mut self, alpha: K, t: usize) -> Self::NextRoundPoly {
        ...
        match self.layer {
            PolynomialLayer::InteractionLayer(layer) => {
                let numerator_0 = Arc::new(layer.numerator_0.as_ref().fix_last_variable::<K>(alpha));
                let denominator_0 = Arc::new(layer.denominator_0.as_ref().fix_last_variable::<K>(alpha));
                let numerator_1 = Arc::new(layer.numerator_1.as_ref().fix_last_variable::<K>(alpha));
                let denominator_1 = Arc::new(layer.denominator_1.as_ref().fix_last_variable::<K>(alpha));

                let new_layer = InteractionLayer { numerator_0, denominator_0, numerator_1, denominator_1 };

                let eq_interaction = Arc::new(self.eq_interaction.as_ref().fix_last_variable(alpha));
            ...
            PolynomialLayer::CircuitLayer(layer) => {
                if layer.num_row_variables == 1 {
                    let numerator_0: Vec<_> =
                        layer.numerator_0.into_iter().map(|mle| mle.fix_last_variable(alpha)).collect();
                    let denominator_0: Vec<_> =
                        layer.denominator_0.into_iter().map(|mle| mle.fix_last_variable(alpha)).collect();
                    let numerator_1: Vec<_> =
                        layer.numerator_1.into_iter().map(|mle| mle.fix_last_variable(alpha)).collect();
                    let denominator_1: Vec<_> =
                        layer.denominator_1.into_iter().map(|mle| mle.fix_last_variable(alpha)).collect();

                    let mut numerator_0_interactions: Vec<_> = numerator_0
                        .into_iter()
                        .flat_map(|mle| mle.eval_at::<K>(&Point::from(vec![])).to_vec())
                        .collect();
                    numerator_0_interactions.resize(1 << layer.num_interaction_variables, K::zero());

                    let mut numerator_1_interactions: Vec<_> = numerator_1
                        .into_iter()
                        .flat_map(|mle| mle.eval_at::<K>(&Point::from(vec![])).to_vec())
                        .collect();
                    numerator_1_interactions.resize(1 << layer.num_interaction_variables, K::zero());

                    let mut denominator_0_interactions: Vec<_> = denominator_0
                        .into_iter()
                        .flat_map(|mle| mle.eval_at::<K>(&Point::from(vec![])).to_vec())
                        .collect();
                    denominator_0_interactions.resize(1 << layer.num_interaction_variables, K::one());

                    let mut denominator_1_interactions: Vec<_> = denominator_1
                        .into_iter()
                        .flat_map(|mle| mle.eval_at::<K>(&Point::from(vec![])).to_vec())
                        .collect();
                    denominator_1_interactions.resize(1 << layer.num_interaction_variables, K::one());
    ...
    fn sum_as_poly_in_last_t_variables(&self, claim: Option<K>, t: usize) -> UnivariatePolynomial<K> {
                    ...
                    itertools::izip!(numerator_0, numerator_1, denominator_0, denominator_1)
                {
                    if let Some(inner) = numerator_0.inner() {
                        ...
                        let (eval_0_chip, eval_half_chip, eq_sum_chip) = inner
                            ...
                            .zip_eq(
                                numerator_1_inner.guts().as_slice().par_chunks(2 * numerator_1_inner.num_polynomials()),
                            )
                            .zip_eq(
                                denominator_0_inner
                                    .guts()
                            ...
                            .map(
                                |((((numer_0_row, numer_1_row), denom_0_row), denom_1_row), eq_row_chunk)| {
                                    let eq_interactions_chip = eq_interaction.guts().as_slice()
                                        [interaction_offset..interaction_offset + numerator_0.num_polynomials()]
                                        .par_iter();

                                    let (numer_0_row_0, numer_0_row_1) = numer_0_row.split_at(numerator_0.num_polynomials());
                                    let (denom_0_row_0, denom_0_row_1) = denom_0_row.split_at(denominator_0.num_polynomials());
                                    let (denom_1_row_0, denom_1_row_1) = denom_1_row.split_at(denominator_1.num_polynomials());
                                    let (numer_1_row_0, numer_1_row_1) = numer_1_row.split_at(numerator_1.num_polynomials());
                                    ...
                                    if numer_0_row.len() == 2 * numerator_0.num_polynomials() {
                                        ...
                                        let numerator_half_eval = numer_0_row_0
                                            ...
                                            .zip_eq(eq_interactions_chip.clone())
                                            .map(
                                                |((((((((n0_0, n1_0), d0_0), d1_0), n0_1), n1_1), d0_1), d1_1), e)| {
                                                    *e * ((*d0_0 + *d0_1) * (*n1_0 + *n1_1) + (*d1_0 + *d1_1) * (*n0_0 + *n0_1))
                                                },
                                            )
                                            .sum::<K>();
                                        let denominator_half_eval = denom_0_row_0
                                            ...
                                            .zip_eq(denom_1_row_1.par_iter())
                                            .zip_eq(eq_interactions_chip.clone())
                                            .map(|((((d0_0, d1_0), d0_1), d1_1), e)| *e * ((*d0_0 + *d0_1) * (*d1_0 + *d1_1)))
                                            .sum::<K>();
                                        let eq_interactions_chip_half =
                                            eq_interactions_chip.map(|e| *e * (eq_row_0 + eq_row_1)).sum::<K>();
                                        (
                                            (lambda * numerator_0_eval + denominator_0_eval) * eq_row_0,
                                            (lambda * numerator_half_eval + denominator_half_eval) * (eq_row_0 + eq_row_1),
                                        ...
                                        let denominator_half_eval = denom_0_row_0
                                            ...
                                            .zip_eq(denom_1_row_0.par_iter())
                                            .zip_eq(eq_interactions_chip.clone())
                                            .map(|((d0, d1), e)| *e * ((*d0 + K::one()) * (*d1 + K::one())))
                                            .sum::<K>();
                                        let eq_interactions_chip_half =
                                            eq_interactions_chip.map(|e| *e * (eq_row_0 + eq_row_1)).sum::<K>();
                                        (
        ...
        // contribution of padded rows for the denominator expression
        ...
        // 8 = 2^3 to account for the evaluations at 1/2 being double their true value.
...
#[cfg(test)]
mod tests {
    use crate::{config::ZkmGlobalContext, prove_gkr_round, GkrCircuitLayer, LogupGkrCpuTraceGenerator};
    ...
    fn random_layer(
        ...
        let padded_numerator_0 = numerator_0
            .iter()
            .map(|mle| PaddedMle::padded_with_zeros(Arc::new(mle.clone()), num_row_variables as u32))
            .collect::<Vec<_>>();
    ...
    fn test_logup_poly_fix_last_variable() {
        ...
        let num_interaction_variables = interaction_counts.iter().sum::<usize>().next_power_of_two().ilog2();
        ...
        let random_point = Point::<EF>::rand(&mut rng, num_row_variables + num_interaction_variables);
        let (interaction_random_point, row_random_point) = random_point.split_at(num_interaction_variables as usize);
        ...
        let mut numerator_0_interactions: Vec<EF> =
            layer.numerator_0.iter().flat_map(|mle| mle.eval_at::<EF>(&row_random_point).to_vec()).collect();
        numerator_0_interactions.resize(1 << layer.num_interaction_variables, EF::zero());

        let mut numerator_1_interactions: Vec<EF> =
            layer.numerator_1.iter().flat_map(|mle| mle.eval_at::<EF>(&row_random_point).to_vec()).collect();
        numerator_1_interactions.resize(1 << layer.num_interaction_variables, EF::zero());

        let mut denominator_0_interactions: Vec<EF> =
            layer.denominator_0.iter().flat_map(|mle| mle.eval_at::<EF>(&row_random_point).to_vec()).collect();
        denominator_0_interactions.resize(1 << layer.num_interaction_variables, EF::one());

        let mut denominator_1_interactions: Vec<EF> =
            layer.denominator_1.iter().flat_map(|mle| mle.eval_at::<EF>(&row_random_point).to_vec()).collect();
        ...
        let mut round_polynomial = first_polynomial.fix_t_variables(*row_random_point.last().unwrap(), 1);
        ...
        for (i, numerator_0_interaction) in numerator_0_interactions.iter().enumerate() {
            assert_eq!(*numerator_0_interaction, interaction_layer.numerator_0.guts().as_slice()[i]);
        }
        for (i, numerator_1_interaction) in numerator_1_interactions.iter().enumerate() {
            assert_eq!(*numerator_1_interaction, interaction_layer.numerator_1.guts().as_slice()[i]);
        }
        for (i, denominator_0_interaction) in denominator_0_interactions.iter().enumerate() {
            assert_eq!(*denominator_0_interaction, interaction_layer.denominator_0.guts().as_slice()[i]);
        }
        for (i, denominator_1_interaction) in denominator_1_interactions.iter().enumerate() {
            assert_eq!(*denominator_1_interaction, interaction_layer.denominator_1.guts().as_slice()[i]);
        }
        ...
        let denominator_0_eval = interaction_layer.denominator_0.eval_at(&interaction_random_point)[0];
        let denominator_1_eval = interaction_layer.denominator_1.eval_at(&interaction_random_point)[0];
    ...
    fn test_logup_poly_sumcheck_circuit_layer() {
        ...
        let claim = {
            ...
            let real_claim = numerator_0
                ...
                .map(|(((n_0, n_1), d_0), d_1)| {
                    let num_padding = vec![F::zero(); ((1 << num_row_variables) - num_rows) * n_0.num_polynomials()];
                    let den_padding = vec![EF::one(); ((1 << num_row_variables) - num_rows) * d_0.num_polynomials()];
                    ...
                    let padded_d1 = d_1
                        ...
                        .chain(den_padding.iter().copied())
                        .collect::<Vec<_>>();
                    let padded_d0 = Mle::from(RowMajorMatrix::new(padded_d0, d_0.num_polynomials()));
                    let padded_d1 = Mle::from(RowMajorMatrix::new(padded_d1, d_1.num_polynomials()));
                    let padded_n0 = Mle::from(RowMajorMatrix::new(padded_n0, n_0.num_polynomials()));
                    let padded_n1 = Mle::from(RowMajorMatrix::new(padded_n1, n_1.num_polynomials()));
        ...
        let (proof, evals) =
            reduce_sumcheck_to_evaluation(vec![round_polynomial], &mut challenger, vec![claim], 1, EF::one());
    ...
    fn test_logup_gkr_circuit_transition() {
        type TraceGenerator = LogupGkrCpuTraceGenerator<KoalaBear, EF, ()>;
        let mut rng = thread_rng();
        ...
        for (next_n0, next_n1, next_d0, next_d1, curr_n0, curr_n1, curr_d0, curr_d1) in itertools::izip!(
            ...
            let _ = next_n0_inner
                ...
                .zip_eq(next_n1_inner.guts().transpose().as_slice().chunks(next_n1.num_real_entries()))
                .zip_eq(
                    curr_n0_inner
                        .guts()
                        .transpose()
                        ...
                        .zip_eq(curr_n1_inner.guts().transpose().as_slice().chunks(curr_n1.num_real_entries())),
                )
                .zip_eq(
                    curr_d0_inner
                        .guts()
                        ...
                        .zip_eq(curr_d1_inner.guts().transpose().as_slice().chunks(curr_d1.num_real_entries())),
                )
                .map(|(((n0_col, n1_col), (curr_n0_col, curr_n1_col)), (curr_d0_col, curr_d1_col))| {
                    let next_n = n0_col.iter().interleave(n1_col.iter()).collect::<Vec<_>>();
                    for (i, ((((next_n_val, curr_n0_val), curr_n1_val), curr_d0_val), curr_d1_val)) in next_n
                        .iter()
                        .copied()
            ...
            let _ = next_d0_inner
                ...
                .zip_eq(next_d1_inner.guts().transpose().as_slice().chunks(next_d1.num_real_entries()))
                .zip_eq(
                    curr_d0_inner
                        .guts()
                        .transpose()
                ...
                .map(|((next_d0_col, next_d1_col), (curr_d0_col, curr_d1_col))| {
                    let next_d = next_d0_col.iter().interleave(next_d1_col.iter()).collect::<Vec<_>>();
                    for (i, ((next_d_val, curr_d0_val), curr_d1_val)) in
                        next_d.iter().copied().zip_eq(curr_d0_col.iter()).zip_eq(curr_d1_col.iter()).enumerate()
                    {
    ...
    fn test_logup_gkr_round_prover() {
        ...
        for layer in layers {
            let round_proof = prove_gkr_round(layer, &eval_point, numerator_eval, denominator_eval, &mut challenger);
            ...
            numerator_eval = round_proof.numerator_0 + (round_proof.numerator_1 - round_proof.numerator_0) * last_coordinate;
            denominator_eval =
                round_proof.denominator_0 + (round_proof.denominator_1 - round_proof.denominator_0) * last_coordinate;
        ...
        for (i, round_proof) in round_proofs.iter().enumerate() {
            ...
            let numerator_sumcheck_eval =
                round_proof.numerator_0 * round_proof.denominator_1 + round_proof.numerator_1 * round_proof.denominator_0;
            let denominator_sumcheck_eval = round_proof.denominator_0 * round_proof.denominator_1;
            let expected_final_eval = eq_eval * (numerator_sumcheck_eval * lambda + denominator_sumcheck_eval);
