...
use crate::config::GKR_GRINDING_BITS;
use crate::{air::MachineAir, prover::Record, record::MachineRecord, Chip, ShardContext, VerifierPublicValuesConstraintFolder};
...
impl<GC: IopCtx, SC: ShardContext<GC>> LogUpGkrVerifier<GC, SC> {
    ...
    pub fn verify_public_values(
        ...
        let betas = partial_lagrange_blocking(beta_seed).into_buffer().into_vec();
    ...
    pub fn verify_logup_gkr(
        ...
        let beta_seed = (0..beta_seed_dim).map(|_| challenger.sample_ext_element::<GC::EF>()).collect::<Point<_>>();
        let pv_challenge = challenger.sample_ext_element::<GC::EF>();
        let cumulative_sum =
            -LogUpGkrVerifier::<GC, SC>::verify_public_values(pv_challenge, &alpha, &beta_seed, public_values)?;

        // Calculate the interaction number.
        let num_of_interactions = shard_chips.iter().map(|c| c.sends().len() + c.receives().len()).sum::<usize>();
        ...
        if numerator.guts().dimensions.sizes() != [expected_size, 1] || denominator.guts().dimensions.sizes() != [expected_size, 1]
        {
        ...
        if output_cumulative_sum != cumulative_sum {
            return Err(LogupGkrVerificationError::CumulativeSumMismatch(output_cumulative_sum, cumulative_sum));
        }
        ...
        let (interaction_point, trace_point) = eval_point.split_at(number_of_interaction_variables as usize);
        ...
        if trace_variables != max_log_row_count {
            return Err(LogupGkrVerificationError::InvalidLastLayerDimension(trace_variables, max_log_row_count));
        }
        ...
        for ((chip, openings), threshold) in shard_chips.iter().zip_eq(chip_openings.values()).zip_eq(degrees.values()) {
            // Observe the opening.
            ...
            {
                let (real_numerator, real_denominator) =
                    interaction.eval(preprocessed_trace_evaluations.as_ref(), main_trace_evaluations, alpha, betas.as_slice());
                let padding_trace_opening = MleEval::from(vec![GC::EF::zero(); main_trace_evaluations.num_polynomials()]);
                ...
                let denominator_eval = real_denominator + (GC::EF::one() - padding_denominator) * geq_eval;
        ...
        if numerator_eval != expected_numerator_eval {
            return Err(LogupGkrVerificationError::NumeratorEvaluationMismatch(numerator_eval, expected_numerator_eval));
        }
        if denominator_eval != expected_denominator_eval {
            return Err(LogupGkrVerificationError::DenominatorEvaluationMismatch(denominator_eval, expected_denominator_eval));
        }
        Ok(())
    }
}
