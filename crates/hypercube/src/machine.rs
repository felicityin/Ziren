...
use crate::{air::MachineAir, record::MachineRecord, Chip};
...
impl<F: Field, A: MachineAir<F>> MachineShape<F, A> {
    ...
    pub fn smallest_cluster(&self, chips: &BTreeSet<Chip<F, A>>) -> Option<&BTreeSet<Chip<F, A>>> {
        self.chip_clusters.iter().filter(|cluster| chips.is_subset(cluster)).min_by_key(|cluster| cluster.len())
    }
}

/// A STARK machine over a fixed set of chips.
#[derive_where(Debug; A: MachineAir<F>)]
#[derive_where(Clone)]
pub struct Machine<F: Field, A> {
    /// The chips that make up the machine, in order of their execution.
    chips: Vec<Chip<F, A>>,
    ...
    A: MachineAir<F>,
{
    ...
    #[must_use]
    pub const fn new(chips: Vec<Chip<F, A>>, num_pv_elts: usize, shape: MachineShape<F, A>) -> Self {
