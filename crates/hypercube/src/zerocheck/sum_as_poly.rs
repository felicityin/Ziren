    ...
    EF: ExtensionField<F>,
{
        ...
        A: for<'b> Air<ConstraintSumcheckFolder<'b, F, K, EF>> + MachineAir<F>,
    {
        ...
        let gkr_powers = self.gkr_powers.clone();
        {
            ...
            let num_preprocessed_columns = preprocessed_values.map_or(0, slop_multilinear::PaddedMle::num_polynomials);
            ...
            let cumul_ys = eq_guts
                ...
                .map(|(i, (eq_chunk, main_chunk))| {
                    ...
                    for (j, (eq, main_row)) in eq_chunk.iter().zip(main_chunk.chunks(num_main_columns * 2)).enumerate() {
                        ...
                        if has_preprocessed_values {
                            let preprocess_chunk_size = values_chunk_size * num_preprocessed_columns;
                            let preprocessed_row_0_start_idx = i * preprocess_chunk_size + 2 * j * num_preprocessed_columns;
                            let preprocessed_row_0 = &preprocessed_values
                                [preprocessed_row_0_start_idx..preprocessed_row_0_start_idx + num_preprocessed_columns];
                            let preprocessed_row_1_start_idx = preprocessed_row_0_start_idx + num_preprocessed_columns;
                            let preprocessed_row_1 = if preprocessed_values.len() != preprocessed_row_1_start_idx {
                                &preprocessed_values
                                    [preprocessed_row_1_start_idx..preprocessed_row_1_start_idx + num_preprocessed_columns]
                            } else {
            ...
            cumul_ys.into_iter().fold((EF::zero(), EF::zero(), EF::zero()), |(y_0, y_2, y_4), (y_0_i, y_2_i, y_4_i)| {
                (y_0 + y_0_i, y_2 + y_2_i, y_4 + y_4_i)
            })
        }
    }
}
    ...
    AirData: for<'b> Air<ConstraintSumcheckFolder<'b, F, K, EF>> + MachineAir<F>,
{
    ...
    let (mut y_0, mut y_2, mut y_4) = poly.air_data.sum_as_poly_in_last_variable::<K, IS_FIRST_ROUND>(
    ...
    let msb_lagrange_eval: EF = poly.eq_adjustment
        * if threshold_half < (1 << (poly.num_variables() - 1)) {
            partial_lagrange.guts().as_buffer()[threshold_half].copy_into_host(partial_lagrange.backend())
        } else {
            EF::zero()
        };
    ...
    let virtual_4 = virtual_geq.fix_last_variable(EF::from_canonical_usize(4)).eval_at_usize(threshold_half);
...
impl<A> ZerocheckCpuProverData<A> {
    ...
    where
        F: Field,
        EF: ExtensionField<F>,
        A: for<'b> Air<ConstraintSumcheckFolder<'b, F, F, EF>> + for<'b> Air<ConstraintSumcheckFolder<'b, F, EF, EF>> + MachineAir<F>,
    {
