...
use slop_sumcheck::{ComponentPolyEvalBackend, SumCheckPolyFirstRoundBackend, SumcheckPolyBackend, SumcheckPolyBase};
...
impl<K, F, EF, AirData> ComponentPolyEvalBackend<ZeroCheckPoly<K, F, EF, AirData>, EF> for CpuBackend
where
    K: Field,
    F: Field,
...
impl<F, EF, A: Send + Sync> SumCheckPolyFirstRoundBackend<ZeroCheckPoly<F, F, EF, A>, EF> for CpuBackend
where
    F: Field,
    EF: ExtensionField<F>,
    A: ZerocheckAir<F, EF>,
{
    ...
    #[inline]
    fn fix_t_variables(poly: ZeroCheckPoly<F, F, EF, A>, alpha: EF, t: usize) -> Self::NextRoundPoly {
        debug_assert_eq!(t, 1);
...
{
    #[inline]
    fn fix_last_variable(poly: ZeroCheckPoly<EF, F, EF, A>, alpha: EF) -> ZeroCheckPoly<EF, F, EF, A> {
        zerocheck_fix_last_variable(poly, alpha)
    }

    #[inline]
    fn sum_as_poly_in_last_variable(poly: &ZeroCheckPoly<EF, F, EF, A>, claim: Option<EF>) -> UnivariatePolynomial<EF> {
        debug_assert!(poly.num_variables() > 0);
