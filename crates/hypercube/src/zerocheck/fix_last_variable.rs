...
>(
    ...
    let preprocessed_columns = poly.preprocessed_columns.as_ref().map(|mle| mle.fix_last_variable(alpha));
    let main_columns = poly.main_columns.fix_last_variable(alpha);

    if poly.main_columns.num_real_entries() == 0 {
        // If the chip is pure padding, its contribution to sumcheck is just zero, we don't need
    ...
    let eq_adjustment = poly.eq_adjustment * ((alpha * last) + (EF::one() - alpha) * (EF::one() - last));

    let has_non_padded_vars = poly.main_columns.num_real_entries() > 1;

    let geq_value =
        if has_non_padded_vars { EF::zero() } else { (EF::one() - poly.geq_value) * alpha + poly.geq_value };
