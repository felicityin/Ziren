use crate::{air::LookupScope, folder::DebugPublicValuesConstraintFolder, prover::Traces, record::MachineRecord, Chip};
    ...
    for row in 0..height {
        for (m, interaction) in sends.iter().chain(receives.iter()).enumerate() {
            ...
            let preprocessed_row = match pre_traces {
                Some(ref t) => {
                    t.inner().as_ref().map_or(empty.as_slice(), |t| t.guts().get(row).unwrap().as_slice())
                }
                None => empty.as_slice(),
            };
            ...
            let main_row = main.inner().as_ref().unwrap().guts().get(row).unwrap().as_slice().to_vec();
            ...
            if !multiplicity_eval.is_zero() {
                ...
                let key = format!("{} {} {}", interaction.scope, interaction.kind, vec_to_string(values));
    ...
    A: MachineAir<F>,
{
    ...
    for chip in chips.iter() {
        ...
        let (_, count) =
            debug_interactions::<F, A>(chip, preprocessed_traces, traces, interaction_kinds.clone(), scope);
        ...
        eprintln!("{} chip has {} distinct events", chip.name(), total_events);
    }
    ...
    for (kind, scope, values, multiplicity) in &folder.interactions {
    ...
    eprintln!("Final counts below.");
    eprintln!("==================");

    let mut any_nonzero = false;
    for (key, (value, chip_values)) in final_map.clone() {
        if !F::is_zero(&value) {
            eprintln!("Interaction key: {} Send-Receive Discrepancy: {}", key, value);
            any_nonzero = true;
            for (chip, chip_value) in chip_values {
                eprintln!(" {} chip's send-receive discrepancy for this key is {}", chip, chip_value);
            }
        }
    }
    ...
    eprintln!("==================");
    if !any_nonzero {
        eprintln!("All chips have the same number of sends and receives.");
    }
