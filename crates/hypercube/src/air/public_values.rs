...
impl<F: PrimeField32> PublicValues<Word<F>, F> {
    ...
    pub fn commit_digest_bytes(&self) -> Vec<u8> {
        self.committed_value_digest.iter().flat_map(|w| w.into_iter().map(|f| f.as_canonical_u32() as u8)).collect_vec()
    }
}
...
mod tests {
    ...
    /// Check that the [`PV_DIGEST_NUM_WORDS`] number matches the zkVM crate's.
    #[test]
