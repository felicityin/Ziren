...
use crate::aot::{get_address_space, get_pc, AotCompiler, AotError};
...
use crate::{
    ...
    Executor, Instruction, MipsAirId, Opcode, Register,
};
...
impl AotCompiler {
    pub fn create_metered_asm(&self) -> Result<String, AotError> {
        ...
        while i < self.program.instructions.len() {
            ...
            asm += "    jae asm_run_end\n";
            // global_clk % shape_check_frequency
            asm += "    # global_clk % shape_check_frequency\n";
            asm += &format!("    mov {REG_HI}, 0\n");
            asm += &format!("    mov {REG_LO_64}, {REG_GLOBAL_CLK}\n");
            asm += &format!("    mov {REG_TMP}, {shape_check_frequency}\n");
            asm += &format!("    div {REG_TMP}\n");
            ...
            asm += &format!("    call {REG_CALLER}\n");
            asm += &format!("    mov {REG_TMP_8L}, al\n");
            asm += &Self::after_call();
            asm += &format!("    mov al, {REG_TMP_8L}\n");
            asm += "    test al, al\n";
            asm += "    jnz asm_run_end\n";
    ...
    fn generate_metered_instruction_asm(
    ...
    fn generate_metered_memory_load_asm(
        instruction: &Instruction,
        pc: u32,
    ) -> Result<String, AotError> {
        let mut asm = String::new();
    ...
    fn generate_metered_memory_store_asm(
        instruction: &Instruction,
        pc: u32,
    ) -> Result<String, AotError> {
        let mut asm = String::new();
    ...
    fn generate_metered_mov_cond_asm(
        instruction: &Instruction,
        pc: u32,
    ) -> Result<String, AotError> {
    ...
    fn generate_metered_syscall_asm(
        instruction: &Instruction,
        pc: u32,
    ) -> Result<String, AotError> {
...
extern "C" fn execute_memory_store(
    executor: &mut Executor,
    instruction: &Instruction,
    pc: u32,
    clk: u32,
...
extern "C" fn execute_memory_load(
    executor: &mut Executor,
    instruction: &Instruction,
    pc: u32,
    clk: u32,
