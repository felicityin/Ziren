...
//! operand values -- no register file, no memory, no `CoreVM` state. This is the boundary between
//! "recomputed live in every pass" (ALU/branch/jump math, HI/LO writes) and "must be logged in the
//! oracle trace" (RAM contents, syscall/precompile writes, hint lengths).
//!
...
/// Same rate, in u32 words -- `ExecutionRecord::split`'s `KECCAK_SPONGE` chunking uses this since
/// `input_len_u32s` is tracked in u32 words, not u64 lanes.
pub(crate) const KECCAK_GENERAL_BLOCK_SIZE_U32S: usize = KECCAK_GENERAL_BLOCK_SIZE_U64S * 2;
...
impl<'a> CoreVM<'a> {
    ...
    /// Current cursor into `stdin` -- see `MinimalExecutor::input_stream_ptr`'s doc comment.
    /// `SplicingVM` snapshots this at each shard boundary alongside `pc()`/`clk()`.
    #[must_use]
