...
use crate::{
    DEFAULT_PC_INC, ExecutionError, ExecutionReport, Instruction, MaximalShapes, MipsAirId, NUM_REGISTERS, Opcode, Program, Register, context::ZKMContext, dependencies::{
    ...
    }, hook::{HookEnv, HookRegistry}, memory::{Entry, Memory}, pad_mips_event_counts, record::{ExecutionRecord, MemoryAccessRecord}, sign_extend, state::{ExecutionState, ForkState}, subproof::SubproofVerifier, syscalls::{Syscall, SyscallCode, SyscallContext, default_syscall_map}, vm::{memory::{AddressMap, GuestMemory, config::{MemoryConfig, RV32_REGISTER_AS}}, state::VmExecState}
};
