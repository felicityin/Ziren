...
impl LinearMemory for MmapMemory {
    ...
    fn new(mut size: usize) -> Self {
        ...
        Self { mmap: MmapMut::map_anon(size).unwrap() }
    }
    ...
    fn fill_zero(&mut self) {
        ...
        // > mappings.
        unsafe {
            let ret = madvise(mmap.as_ptr() as *mut libc::c_void, mmap.len(), MADV_DONTNEED);
            if ret != 0 {
    ...
    unsafe fn swap<BLOCK: Copy>(&mut self, start: usize, values: &mut BLOCK) {
        ...
        core::ptr::swap(self.as_mut_ptr().add(start) as *mut BLOCK, values as *mut BLOCK);
    }
