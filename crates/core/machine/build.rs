...
mod sys {
    ...
    fn patch_generated_cbindgen_header(header_path: &Path) -> io::Result<()> {
        if !header_path.exists() {
            return Ok(());
        }

        let original = fs::read_to_string(header_path)?;
        if original.contains("constexpr static const uintptr_t U64_LIMBS = 4;") {
            return Ok(());
        }
        ...
        for line in original.lines() {
            if line.contains("constexpr static const uintptr_t U64_LIMBS") {
                continue;
            }

            out.push(line.to_owned());
            if !inserted && line.contains("namespace zkm_core_machine_sys") {
                out.push(String::new());
                out.push("constexpr static const uintptr_t U64_LIMBS = 4;".to_owned());
                out.push(String::new());
                inserted = true;
            }
        }
            ...
            .generate()
        {
            Ok(bindings) => {
                ...
                if bindings.write_to_file(&header_path) {
                    patch_generated_cbindgen_header(&header_path).unwrap();
