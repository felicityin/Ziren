...
pub mod tests {
    ...
    /// i.e. 5, since `3*5 = 15 = 2*7 + 1`) as public values before halting. Mirrors
    /// `zkm_core_executor::programs::tests::hook_fp_inverse_program` exactly (duplicated here
    /// rather than reused, matching this file's existing convention of its own synthetic
    /// programs, separate from the executor crate's).
    #[must_use]
    #[allow(clippy::unreadable_literal)]
    pub fn hook_fp_inverse_program() -> Program {
        use zkm_core_executor::{syscalls::SyscallCode, FD_FP_INV};
        ...
        let instructions = vec![
            ...
            Instruction::new(Opcode::SYSCALL, 2, 4, 5, false, false),
            // A real load to force the hinted word to materialize into memory before `WRITE`'s
            // own untracked byte peek reads it.
