...
mod tests {
    ...
    fn generate_test_execution_record(pass: bool) -> ExecutionRecord {
        ...
        let lo_ptr_memory =
            MemoryReadRecord { value: lo_ptr, timestamp: hi_ts, prev_timestamp: lo_ts };
        let hi_ptr_memory =
            MemoryReadRecord { value: hi_ptr, timestamp: hi_ts, prev_timestamp: lo_ts };
    ...
    fn test_uint256_mul() {
        utils::setup_logger();
        let program = Program::from(U256XU2048_MUL_ELF).unwrap();
        run_test_io(program, ZKMStdin::new()).unwrap();
    }
