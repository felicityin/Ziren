...
/// `StoreWordUnalignedChip`). Like [`super::WordAddressOperation`], verifies the sum via the ALU
/// table, range-checks it into the field, and checks it's at least `NUM_REGISTERS` -- but instead
/// of asserting word alignment, it witnesses the low 2 bits (`addr_ls_two_bits`) and the aligned
/// address (`addr_aligned`) so callers can mux the accessed byte/halfword within the aligned word.
