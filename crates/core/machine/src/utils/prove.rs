// This entire file is the old FRI-backed proving-orchestration driver
// (StarkMachine/MachineProver/CpuProver-based shard splitting, checkpoint
// replay, deferred-proof handling, prove/run_test/run_test_io/run_test_core/
// run_test_machine, plus the generic uni_stark_prove/uni_stark_verify wrappers).
// It cannot compile as-is: MipsAir<F> no longer implements zkm_stark::air::MachineAir
// (its derive now targets zkm_hypercube::air::MachineAir), and none of the ported
// chips implement the old Air<SymbolicAirBuilder>/Air<ProverConstraintFolder>/etc
// folder bounds these functions require. Commented out in full (not deleted) until
// zkm-hypercube grows its own shard-proving driver (task: 更新 prover/sdk/cli).
//
