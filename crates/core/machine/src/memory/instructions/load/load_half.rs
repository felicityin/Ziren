    ...
    AB: ZKMCoreAirBuilder + AirBuilderWithPublicValues,
{
    fn eval(&self, builder: &mut AB) {
        ...
        // A negative halfword's sign-extended 32-bit value is just `unsigned_mem_val`'s low 2
        // (real) bytes with the upper 2 (always-zero) bytes replaced by `0xFF` -- a direct byte
        // assertion, not an arithmetic dependency on another chip's SUB circuit.
        let sign_extended_value = Word([
            local.unsigned_mem_val[0].into(),
            local.unsigned_mem_val[1].into(),
