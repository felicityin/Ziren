...
//! `main.rs` feeds the guest via stdin, as raw `.bin` files next to the compiled guest ELF --
//! for `zkm-core-machine`'s `mips::tests::debug_real_tendermint_guest_with_stdin_interactions_balance`
//! to load directly (that test can't depend on `tendermint-light-client-verifier`/`serde_cbor`
//! itself without pulling them into the machine crate).
