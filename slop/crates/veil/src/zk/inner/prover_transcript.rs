...
pub struct ProverElement<K: FieldAlgebra> {
    value: K,
    index: TranscriptIndex<K>,
}

impl<K: FieldAlgebra + Copy> ProverElement<K> {
...
#[derive(Debug, Clone)]
pub struct ProverLinExpression<K: FieldAlgebra> {
    value: K,
    expr: TranscriptLinConstraint<K>,
}

impl<K: FieldAlgebra + Copy> ProverLinExpression<K> {
...
impl<K: FieldAlgebra + Copy> From<ProverElement<K>> for TranscriptIndex<K> {
    fn from(element: ProverElement<K>) -> Self {
        element.index
    }
}

impl<K: FieldAlgebra + Copy> From<ProverElement<K>> for TranscriptLinConstraint<K> {
    fn from(element: ProverElement<K>) -> Self {
        element.index.into()
    }
}

impl<K: FieldAlgebra + Copy> From<ProverElement<K>> for ProverLinExpression<K> {
    fn from(element: ProverElement<K>) -> Self {
...
impl<K: FieldAlgebra + Copy> Mul<K> for ProverElement<K> {
    type Output = ProverLinExpression<K>;
...
impl<K: FieldAlgebra + Copy> From<K> for ProverLinExpression<K> {
    fn from(value: K) -> Self {
        ProverLinExpression::new(value, value.into())
    }
}

impl<K: FieldAlgebra + Copy> From<ProverLinExpression<K>> for TranscriptLinConstraint<K> {
...
impl<K: FieldAlgebra + Copy> Mul<K> for ProverLinExpression<K> {
    type Output = Self;
...
impl<K: FieldAlgebra + Copy> MulAssign<K> for ProverLinExpression<K> {
...
impl<K: FieldAlgebra + Copy> ZkElement<K> for ProverElement<K> {
    type LinExpr = ProverLinExpression<K>;
}

impl<K: FieldAlgebra + Copy> ZkLinExpression<K, ProverElement<K>> for ProverLinExpression<K> {}
