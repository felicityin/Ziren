    ...
    K: FieldAlgebra + Copy,
{
    ...
    pub fn to_flat_index<T: slop_algebra::FieldAlgebra>(
        &self,
...
pub struct TranscriptIndex<K: FieldAlgebra> {
    inner: [usize; 2],
    _phantom: PhantomData<K>,
}

impl<K: FieldAlgebra> std::fmt::Debug for TranscriptIndex<K> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TranscriptIndex").field("inner", &self.inner).finish()
    }
}

impl<K: FieldAlgebra> From<[usize; 2]> for TranscriptIndex<K> {
    fn from(index: [usize; 2]) -> Self {
        Self { inner: index, _phantom: PhantomData }
    }
}

impl<K: FieldAlgebra> From<TranscriptIndex<K>> for [usize; 2] {
...
#[derive(Debug, Clone)]
pub struct TranscriptLinConstraint<K: FieldAlgebra>(Vec<(K, TranscriptIndex<K>)>);

/// Get an expression representing the constant value K
impl<K: FieldAlgebra> From<K> for TranscriptLinConstraint<K> {
...
impl<K: FieldAlgebra> Default for TranscriptLinConstraint<K> {
    fn default() -> Self {
        Self(vec![])
    }
}

impl<K: FieldAlgebra + Copy> TranscriptLinConstraint<K> {
...
impl<K: FieldAlgebra + Copy> From<TranscriptIndex<K>> for TranscriptLinConstraint<K> {
    fn from(index: TranscriptIndex<K>) -> Self {
...
impl<K: FieldAlgebra + Copy> Mul<K> for TranscriptLinConstraint<K> {
    type Output = Self;
...
impl<K: FieldAlgebra + Copy> Mul<K> for TranscriptIndex<K> {
    type Output = TranscriptLinConstraint<K>;
...
impl<K: FieldAlgebra + Copy> MulAssign<K> for TranscriptLinConstraint<K> {
...
#[derive(Debug, Clone)]
pub struct TranscriptMulConstraint<K: FieldAlgebra>(pub [TranscriptLinConstraint<K>; 3]);

impl<K: FieldAlgebra> TranscriptMulConstraint<K> {
