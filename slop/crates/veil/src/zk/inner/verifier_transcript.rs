use slop_algebra::FieldAlgebra;

use super::{
    ExpressionIndex, TranscriptIndex, TranscriptLinConstraint, ZkElement, ZkExpression, ZkIopCtx,
    ZkLinExpression, ZkVerificationContext,
};

pub type VerifierElement<K> = TranscriptIndex<K>;

pub type VerifierLinExpression<K> = TranscriptLinConstraint<K>;

impl<K: FieldAlgebra + Copy> ZkElement<K> for VerifierElement<K> {
    type LinExpr = VerifierLinExpression<K>;
}

impl<K: FieldAlgebra + Copy> ZkLinExpression<K, VerifierElement<K>> for VerifierLinExpression<K> {}

impl<K: FieldAlgebra + Copy> From<[usize; 2]> for ZkExpression<K, VerifierElement<K>> {
    fn from(indices: [usize; 2]) -> Self {
        let elt: VerifierElement<K> = indices.into();
        ZkExpression::Element(elt)
    }
}

/// Type alias for expression indices in the verifier context.
///
/// # Type Parameters
/// * `GC` - The ZK IOP context type; `GC::PcsProof` is the PCS proof type.
#[allow(type_alias_bounds)]
pub type VerifierValue<GC: ZkIopCtx> = ExpressionIndex<GC::EF, ZkVerificationContext<GC>>;
