...
use slop_algebra::{
    interpolate_univariate_polynomial, FieldExtensionAlgebra, FieldAlgebra, UnivariatePolynomial,
};
...
fn ef_mle_to_base(mle: &Mle<EF>) -> Mle<F> {
    let data: Vec<F> =
        mle.guts().as_slice().iter().map(|x| FieldExtensionAlgebra::as_base_slice(x)[0]).collect();
