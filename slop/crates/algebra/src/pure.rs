...
pub trait Algebra<R>: FieldAlgebra + Module<R> {}

impl<R, E> Algebra<R> for E where E: FieldAlgebra + Module<R> {}
...
impl<F, E> FieldAlgebra for Dorroh<F, E>
where
    F: Field,
    E: Module<F> + Mul<Output = E> + Add<Output = E> + Sub<Output = E> + Debug,
{
    ...
    fn generator() -> Self {
        Self::Constant(<F as Field>::generator())
    }
}
