...
use slop_algebra::{FieldExtensionAlgebra, FieldAlgebra, Field};
...
impl<F: FieldAlgebra + 'static + Send + Sync> JaggedLittlePolynomialVerifierParams<F> {
    ...
    pub fn full_jagged_little_polynomial_evaluation<
        EF: FieldExtensionAlgebra<F> + 'static + Send + Sync,
    >(
        &self,
        z_row: &Point<EF>,
        z_col: &Point<EF>,
...
pub struct BranchingProgram<K: FieldAlgebra> {
    z_row: Point<K>,
    z_index: Point<K>,
...
impl<K: FieldAlgebra + 'static> BranchingProgram<K> {
